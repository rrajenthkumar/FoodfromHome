defmodule FoodFromHomeWeb.AuthControllerTest do
  ...
  @auth0 Application.compile_env(:food_from_home, :auth0)
  ...
  test "POST /auth/sign-in should return 400 when payload is not valid", %{conn: conn} do
    response =
      conn
      |> put_req_header("accept", "application/json")
      |> post("/auth/sign-in", username: "test@rajenth.de")
      |> json_response(400)
