defmodule FoodFromHomeWeb.UserControllerTest do
  ...
  describe "index" do
    ...
    test "lists users filtered based on query parameters", %{conn: conn} do
      # user_1 is of type :buyer by default
      _user_1 = UserRepoFixtures.user_fixture()
  ...
  describe "create" do
    ...
    test "creates user, seller and renders user when data is valid for seller user type", %{
      conn: conn
    } do
      conn = post(conn, ~p"/api/v1/users", user: @create_attrs_with_seller_data)

      %{"id" => user_id, "seller" => %{"id" => seller_id, "link" => seller_route}} =
  ...
  describe "update" do
    ...
    test "renders error in case of user_type field update", %{
      conn: conn,
