defmodule FoodFromHome.FoodMenus.FoodMenuRepoTest do
  ...
  setup do
    ...
    valid_attrs_1 = %{
      ...
      price: 5,
      valid_until: ~U[2100-11-25 14:28:00Z]
    }
  ...
  test "create_food_menu/1 with valid data creates a food menu" do
    ...
    valid_attrs = %{
      ...
      price: 9.5,
      valid_until: ~U[2100-11-30 14:28:00Z]
    }
  ...
  test "update_food_menu/2 with invalid data returns error changeset", %{food_menu_2: food_menu} do
    assert {:error, %Ecto.Changeset{}} =
             FoodMenuRepo.update_food_menu(@invalid_attrs, food_menu.id)
  ...
  describe "list_food_menus/1" do
    test "lists all food menus", %{
      ...
      assert FoodMenuRepo.list_food_menus(_filter_params = %{}) == [
               food_menu_1,
               food_menu_2,
               food_menu_3
             ]
    ...
    test "lists only food menus for a given seller_id", %{
      food_menu_2: food_menu_2,
      food_menu_3: food_menu_3
    } do
      assert FoodMenuRepo.list_food_menus(_filter_params = %{seller_id: food_menu_3.seller_id}) ==
               [food_menu_2, food_menu_3]
    end

    test "lists only active food menus for a given seller_id", %{food_menu_3: food_menu_3} do
      assert FoodMenuRepo.list_food_menus(
               _filter_params = %{seller_id: food_menu_3.seller_id, active: "true"}
             ) == [food_menu_3]
    end
  end
end
