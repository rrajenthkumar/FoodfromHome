defmodule FoodFromHome.Deliveries.Services.CreateDeliveryWithSellerPosition do
  ...
  def call(order = %Order{status: :reserved_for_pickup}, %User{id: deliverer_user_id, user_type: :deliverer}) do
    %User{address: seller_address} = Users.find_seller_user_from_order!(order)

    case GoogleGeocoding.get_position(seller_address) do
      {:ok, %Geo.Point{} = seller_position} -> Deliveries.create(order, %{deliverer_user_id: deliverer_user_id, current_position: seller_position})
      error -> error
    end
  end
end
