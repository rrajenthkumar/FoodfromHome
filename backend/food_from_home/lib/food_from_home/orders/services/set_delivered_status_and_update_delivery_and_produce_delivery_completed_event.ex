        ...
        case add_delivery_time(order) do
          ...
          {:error, delivery_time_addition_error_reason} ->
            ...
            case Orders.update(order, %{status: :on_the_way}) do
              ...
              {:error, reason} ->
                {:error, 500,
                 "Delivery time addition to the associated delivery failed due to the following reason: #{delivery_time_addition_error_reason} and the eventual order status update rollback also failed due to the following reason: #{reason}."}
            end
        end
