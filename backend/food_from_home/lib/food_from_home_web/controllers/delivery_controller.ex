defmodule FoodFromHomeWeb.DeliveryController do
  ...
  def show(conn = %{assigns: %{current_user: %User{} = current_seller_or_deliverer_user}}, %{"order_id" => order_id}) do
  ...
  def update(conn = %{assigns: %{current_user: %User{user_type: :deliverer} = current_user}}, %{"order_id" => order_id, "delivery" => delivery_params}) do
    delivery = Deliveries.get_with_order_id!(order_id)
  ...
  defp delivery_related_to_current_user?(%User{id: current_user_id, user_type: :deliverer}, %Delivery{deliverer_user_id: deliverer_user_id}) do
